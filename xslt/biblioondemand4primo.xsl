...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
         ...
         <ListRecords>
            <xsl:for-each select="marc:record">
               ...
               <xsl:call-template name="copyrecord">
                  ...
                  <xsl:with-param name="provider" select="'biblioondemand'"/>
               </xsl:call-template>
            </xsl:for-each>
         </ListRecords>
      </OAI-PMH>
  ...
  <xsl:template name="copyrecord">
      ...
      <xsl:variable name="id" select="concat($provider, replace($isbn,'-',''))"/>
      <!--xsl:variable name="file_name">inln.<xsl:value-of select="$id"/>.xml</xsl:variable-->
      <record>
         <header>
            <identifier>
               <xsl:value-of select="lower-case($provider)"/>-publish:<xsl:value-of select="$id"/>
            </identifier>
         </header>
         <metadata>
            <record>
  ...
  <!-- When matching marc:datafield[@tag='215']: do nothing -->
  <xsl:template match="marc:datafield[@tag='215']"/>
  <!-- When matching marc:subfield[@code='3']: do nothing -->
  <xsl:template match="marc:datafield/marc:subfield[@code='3']"/>
